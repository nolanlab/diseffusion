 ...
 * @author Nikolay
 */
public class GenerateSynthDatasetTrajectory_Diseffusion {
    ...
    public GenerateSynthDatasetTrajectory() {
    }
        ...
        for (int sz = 1000; sz <= 1000; sz += 10) {
            ...
            i: for (int i = 0; i < SIZES.length; i++) {
                ...
                for (int j = 0; j < dim; j++) {
                    ...
                    if (i > 0 && Math.random() > 0.3) {
                        ...
                        if(i<SIZES.length/2) {
                            ...
                            sigma[i].setQuick(j, j, sigma[i - 1].get(j, j));
                        }else{
                                mu[i].setQuick(j, mu[i - 2].getQuick(j));
                                sigma[i].setQuick(j, j, sigma[i - 2].get(j, j));
                ...
                if (i > 0) {
                    ...
                    if(i<SIZES.length/2) {
                        distance = MatrixOp.getEuclideanDistance(mu[i].toArray(), mu[i - 1].toArray());
                    }else{
                        distance = MatrixOp.getEuclideanDistance(mu[i].toArray(), mu[i - 2].toArray());
                    }
                ...
                gen[i] = new MultinormalPCAGen(new NormalGen(new MRG31k3p()), mu[i].toArray(), sigma[i].toArray());
 ...
 /*
            ...
            for (int i = 0; i < gen.length; i++) {
                ...
                for (int j = 0; j < SIZES[i]; j++) {
                    ...
                    for (int k = 0; k < SIZES.length; k++) {
                        dens += dist[k].density(vec) * (SIZES[k] / (double) sumSizes);
                    }*/
            ...
            Dataset ds = new Dataset("Synth_Student_1000comp_trajectory_50d_Fork", dp.toArray(new Datapoint[dp.size()]), paramNames, new String[]{"Density", "ComponentID"});
